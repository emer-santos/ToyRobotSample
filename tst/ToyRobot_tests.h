...
TEST_CASE("Toy Robot - Move")
{
    ...
    CHECK(robot.report() == std::make_tuple(3, 3, Direction::North));

    robot.moveForward();
    CHECK(robot.report() == std::make_tuple(3, 4, Direction::North));

    robot.rotateRight();
    robot.moveForward();
    CHECK(robot.report() == std::make_tuple(4, 4, Direction::East));

    robot.rotateRight();
    robot.moveForward();
    CHECK(robot.report() == std::make_tuple(4, 3, Direction::South));

    robot.rotateRight();
    robot.moveForward();
    CHECK(robot.report() == std::make_tuple(3, 3, Direction::West));
}
...
TEST_CASE("Toy Robot - Bounds Check")
{
    ...
    CHECK(robot.report() == std::make_tuple(4, 0, Direction::South));

    robot.place(0, 4, Direction::West);
    CHECK(robot.moveForward() == false);
    CHECK(robot.report() == std::make_tuple(0, 4, Direction::West));

    robot.place(0, 4, Direction::North);
    CHECK(robot.moveForward() == false);
    CHECK(robot.report() == std::make_tuple(0, 4, Direction::North));

    robot.place(4, 0, Direction::East);
    CHECK(robot.moveForward() == false);
    CHECK(robot.report() == std::make_tuple(4, 0, Direction::East));
